...
public = public.slice(0,public.length - 2).join('/')
let publicPath = path.join(public, 'dist');
...
app.get('/characters/:name', async (req, res) => {
    CharacterModel.find({ name: req.params.name }, (err, docs) => {
        if (docs.length != 0) {
            client.setex(req.baseUrl, 3000, JSON.stringify(docs[0]))
            return res.status(200).json(docs[0])
        } else {
            CharacterModel.find({ name: { $regex: new RegExp(`^${req.params.name[0]}.*`), $options: 'i' } }, (err, doc) => {
                client.setex(req.baseUrl, 3000, JSON.stringify(doc[0]))
